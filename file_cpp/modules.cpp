...
string random_string(unsigned long length) {
    string rnd_str = "";
    default_random_engine string_generator;
...
int main() {
    auto start = chrono::steady_clock::now();
    tuple<double, unsigned long> gay_tuple = generator_cpp("Z:\\test-simulator\\test",false,false,5);
    double gay1 = sorter_cpp("Z:\\test-simulator\\test",false,false);
    double gay2 = remover_cpp("Z:\\test-simulator\\test",false,false);
    auto end = chrono::steady_clock::now();
    auto duration = chrono::duration_cast<chrono::nanoseconds>(end - start);
    double duration_num = duration.count() / pow(10,9);
